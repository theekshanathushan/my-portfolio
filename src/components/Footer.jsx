import React from 'react';
import { ArrowUp, Heart, Mail } from 'lucide-react';

const Github = (props) => (
  <svg viewBox="0 0 24 24" width={props.size || 24} height={props.size || 24} stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round" {...props}>
    <path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4" />
    <path d="M9 18c-4.51 2-5-2-7-2" />
  </svg>
);

const Linkedin = (props) => (
  <svg viewBox="0 0 24 24" width={props.size || 24} height={props.size || 24} stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round" {...props}>
    <path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z" />
    <rect x="2" y="9" width="4" height="12" />
    <circle cx="4" cy="4" r="2" />
  </svg>
);

const Twitter = (props) => (
  <svg viewBox="0 0 24 24" width={props.size || 24} height={props.size || 24} stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round" {...props}>
    <path d="M22 4s-.7 2.1-2 3.4c1.6 10-9.4 17.3-18 11.6 2.2.1 4.4-.6 6-2C3 15.5.5 9.6 3 5c2.2 2.6 5.6 4.1 9 4-.9-4.2 4-6.6 7-3.8 1.1 0 3-1.2 3-1.2z" />
  </svg>
);


export default function Footer() {
  const scrollToTop = () => {
    window.scrollTo({ top: 0, behavior: 'smooth' });
  };

  return (
    <footer className="footer-wrap">
      <div className="container footer-container">
        
        {/* Upper Layer */}
        <div className="footer-upper">
          <div className="footer-brand">
            <span className="footer-logo">PORT<span className="text-glow-accent">FOLIO</span></span>
            <p className="footer-tagline">Crafting modern interactive experiences with precision & care.</p>
          </div>

          <div className="footer-socials">
            <a href="https://github.com/G-T-Thushan" target="_blank" rel="noreferrer" className="footer-social-icon" title="GitHub">
              <Github size={18} />
            </a>
            <a href="https://www.linkedin.com/in/theekshana-thushan-57b866328" target="_blank" rel="noreferrer" className="footer-social-icon" title="LinkedIn">
              <Linkedin size={18} />
            </a>
            <a href="https://twitter.com" target="_blank" rel="noreferrer" className="footer-social-icon" title="Twitter">
              <Twitter size={18} />
            </a>
            <a href="mailto:theekshanathushan89@gmail.com" className="footer-social-icon" title="Email">
              <Mail size={18} />
            </a>
          </div>
        </div>

        {/* Divider line */}
        <div className="footer-divider"></div>

        {/* Lower Layer */}
        <div className="footer-lower">
          <span className="footer-copy">
            © {new Date().getFullYear()} Theekshana Thushan. All rights reserved.
          </span>

          <span className="footer-signature">
            Designed & Built with <Heart size={12} className="heart-icon" /> by Theekshana
          </span>

          <button 
            className="back-to-top-btn glass-card" 
            onClick={scrollToTop} 
            title="Back to Top"
          >
            <ArrowUp size={16} />
          </button>
        </div>

      </div>

      <style>{`
        .footer-wrap {
          background: rgba(4, 4, 8, 0.5);
          border-top: 1px solid var(--border-glass);
          padding: 60px 0 30px;
          position: relative;
          z-index: 10;
        }
        .footer-container {
          display: flex;
          flex-direction: column;
          gap: 32px;
        }
        .footer-upper {
          display: flex;
          justify-content: space-between;
          align-items: center;
          gap: 24px;
          flex-wrap: wrap;
        }
        @media (max-width: 576px) {
          .footer-upper {
            flex-direction: column;
            text-align: center;
          }
        }
        .footer-brand {
          display: flex;
          flex-direction: column;
          gap: 8px;
          text-align: left;
        }
        @media (max-width: 576px) {
          .footer-brand {
            text-align: center;
            align-items: center;
          }
        }
        .footer-logo {
          font-family: var(--font-heading);
          font-weight: 700;
          font-size: 18px;
          color: var(--text-white);
          letter-spacing: 0.05em;
        }
        .footer-tagline {
          color: var(--text-muted);
          font-size: 13px;
          max-width: 300px;
        }
        .footer-socials {
          display: flex;
          gap: 12px;
        }
        .footer-social-icon {
          width: 38px;
          height: 38px;
          border-radius: 50%;
          background: rgba(255,255,255,0.02);
          border: 1px solid var(--border-glass);
          display: flex;
          align-items: center;
          justify-content: center;
          color: var(--text-muted);
          transition: all 0.3s;
          text-decoration: none;
        }
        .footer-social-icon:hover {
          color: var(--accent);
          border-color: var(--accent);
          transform: translateY(-2px);
          box-shadow: 0 4px 10px rgba(var(--accent-rgb), 0.15);
        }
        .footer-divider {
          height: 1px;
          background: var(--border-glass);
          width: 100%;
        }
        .footer-lower {
          display: flex;
          justify-content: space-between;
          align-items: center;
          flex-wrap: wrap;
          gap: 16px;
        }
        @media (max-width: 576px) {
          .footer-lower {
            flex-direction: column;
            text-align: center;
          }
        }
        .footer-copy {
          font-size: 13px;
          color: var(--text-dark);
        }
        .footer-signature {
          font-size: 13px;
          color: var(--text-muted);
          display: inline-flex;
          align-items: center;
          gap: 4px;
        }
        .heart-icon {
          color: #ef4444;
          animation: beat 1.5s infinite alternate;
        }
        .back-to-top-btn {
          width: 36px;
          height: 36px;
          border-radius: 50%;
          display: flex;
          align-items: center;
          justify-content: center;
          color: var(--text-muted);
          background: rgba(255, 255, 255, 0.02);
          border: 1px solid var(--border-glass);
          transition: all 0.2s;
        }
        .back-to-top-btn:hover {
          color: var(--accent);
          border-color: var(--accent);
          transform: translateY(-3px);
          box-shadow: 0 4px 12px rgba(var(--accent-rgb), 0.2);
        }

        @keyframes beat {
          to { transform: scale(1.2); }
        }
      `}</style>
    </footer>
  );
}
